...
def load_data(sample_ratio=1.0):
    ...
    train_data_loader = DataLoader(dataset=ImageDataSet(data=train_data,
                                                        data_shape=(IMAGE_WIDTH, IMAGE_HEIGHT, IMAGE_CHANNEL),
                                                        is_transform=False),
                                   batch_size=BATCH_SIZE,
                                   shuffle=True)

    valid_data_loader = DataLoader(dataset=ImageDataSet(data=valid_data,
                                                        data_shape=(IMAGE_WIDTH, IMAGE_HEIGHT, IMAGE_CHANNEL),
                                                        is_transform=False),
                                   batch_size=BATCH_SIZE,
                                   shuffle=True)

    test_data_loader = DataLoader(dataset=ImageDataSet(data=test_data,
                                                       data_shape=(IMAGE_WIDTH, IMAGE_HEIGHT, IMAGE_CHANNEL),
                                                       is_transform=False),
                                  batch_size=BATCH_SIZE,
                                  shuffle=True)
